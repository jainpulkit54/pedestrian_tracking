...
parser.add_argument("--network_config", type = str, default = "config/yolov3-MOT.cfg", help = "Patch to the file containing the network definition")
parser.add_argument("--use_coco2014_weights", type = str, default = "False", help = "Set this to true if you want to use the pretrained COCO2014 weights else use the custom weights file")
parser.add_argument("--coco2014_weights_file", type = str, default = "checkpoints_coco2014/yolov3.weights", help = "Path to the weights file")
parser.add_argument("--custom_weights_file", type = str, default = "checkpoints_800_mot1720Det/model_epoch_29.pth", help = "Path to the weights file")
parser.add_argument("--class_path", type = str, default = "data/classes.names", help = "Path to the class label file")
parser.add_argument("--conf_thresh", type = float, default = 0.7, help = "Object Confidence Threshold")
...
if args.use_coco2014_weights == 'True':
	args.use_coco2014_weights = True
else:
	args.use_coco2014_weights = False

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
os.makedirs('output_MOT17-02', exist_ok = True)
...
if args.use_coco2014_weights:
	model.load_darknet_weights(args.coco2014_weights_file)
else:
	checkpoint = torch.load(args.custom_weights_file)
...
for index, image in enumerate(dataloader):
	...
	try:
		...
		fig.savefig('output_MOT17-02/' + str(index + 1).zfill(6) + '.jpg', bbox_inches = 'tight', pad_inches = 0)
