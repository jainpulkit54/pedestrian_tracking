...
class Tracker:
	...
	def step(self, frame, orig_image, points_old_flag, frame_num):
		...
		try:
			...
			if points_old_flag is False:
				...
				self.old_box_dim = [[w, h] for w, h in zip(box_width, box_height)]
				self.old_box_dim = np.array(self.old_box_dim)
				...
				if ((frame_num + 1) % 3) == 0:
					...
					self.points_old = self.points_new.copy()
					self.points_new = self.points_new.reshape(-1, 2)
					self.points_new = np.concatenate((self.points_new, self.old_box_dim), axis = 1)
