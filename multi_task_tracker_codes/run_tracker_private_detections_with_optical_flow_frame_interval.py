...
from utils import *
from mytracker_private_detections_with_optical_flow_frame_interval import *

parser = argparse.ArgumentParser()
parser.add_argument("--batch_size", type = int, default = 1, help = "Image Batch Size")
parser.add_argument("--image_folder", type = str, default = "/home/pulkit/Desktop/MTP/input_images/MOT17/", help = "Path to the dataset folder")
...
parser.add_argument("--tracks_file_name", type = str, default = "MOT17.txt", help = "Enter the file name in which you want to store the object trajectories")
parser.add_argument("--output_images_folder_name", type = str, default = "outputs/MOT17/private_detections/MOT17_private_detections", help = "Enter the folder name in which you want to save the images with tracks")
...
for index, image in enumerate(dataloader):
	...
	if index == 0:
		...
		tic = time.time()
		with torch.no_grad():
			trackers = tracker.step(img, old_orig_image, False, index)
		toc = time.time()
		...
		tic = time.time()
		with torch.no_grad():
			trackers = tracker.step(img, current_orig_image, True, index)
		toc = time.time()
		print('Processing Time:', (toc - tic))
