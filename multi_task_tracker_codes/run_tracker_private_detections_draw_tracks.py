...
parser.add_argument("--image_folder", type = str, default = "/home/pulkit/Datasets/MOT_Datasets/Detection_Datasets/MOT17Det/train/MOT17-04/img1/", help = "Path to the dataset folder")
...
parser.add_argument("--weights_path_reid", type = str, default = "checkpoints_market1501_batchhard/model_epoch_50.pth", help = "Path to the reid weight file")
...
parser.add_argument("--tracks_folder_name", type = str, default = "outputs/tracks_private_detections", help = "Enter the name of the folder where you want to save the tracks obtained")
parser.add_argument("--tracks_file_name", type = str, default = "MOT17-04.txt", help = "Enter the file name in which you want to store the object trajectories")
parser.add_argument("--output_images_folder_name", type = str, default = "outputs/private_detections/MOT17-04", help = "Enter the folder name in which you want to save the images with tracks")
	...
	if index == 0:
		...
		for track in trackers:
			...
			cv2.putText(img = current_orig_image, text = str('%d'%object_num), org = (x_min, y_min), fontFace = cv2.FONT_HERSHEY_SIMPLEX,
						fontScale = 0.50, color = [0, 0, 0], thickness = 1)
			print('%d,%d,%.2f,%.2f,%.2f,%.2f,-1,-1,-1'%((index+1), object_num, x_min, y_min, width, height),file = file_out)
		...
		if len(old_trackers) >= len(trackers):
			for track_old in old_trackers:
				...
				for track_new in trackers:
					...
					if id_old == id_new:
						mask = cv2.line(mask, (x_center_old, y_center_old), (x_center_new, y_center_new), colors[id_old % 512, :].tolist(), 2)
						frame = cv2.circle(current_orig_image, (x_center_new, y_center_new), 5, colors[id_old % 512, :].tolist(), thickness = -1)
			...
			for track_new in trackers:
				...
				for track_old in old_trackers:
					...
					id_old = int(track_old[4])
					if id_old == id_new:
						mask = cv2.line(mask, (x_center_old, y_center_old), (x_center_new, y_center_new), colors[id_new % 512, :].tolist(), 2)
						frame = cv2.circle(current_orig_image, (x_center_new, y_center_new), 5, colors[id_new % 512, :].tolist(), thickness = -1)
