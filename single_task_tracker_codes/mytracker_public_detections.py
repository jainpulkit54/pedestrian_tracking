...
class Tracker:
	...
	def __init__(self, do_reid, person_reid, public_detections_file):
		...
		self.do_reid = do_reid # Whether to do Person Reid or not
		self.reid_cosine_dist_threshold = 0.5 # Threshold for cosine distance between track and detection after which they are considered to be different
		...
		self.max_age = 50 # The number of frames for which each tracker will be kept alive before deletion in case of missing detections
	...
	def associate_detections_to_targets_with_reid(self, detections, trackers):
		...
		cosine_distance_matrix = np.zeros([no_of_detections, no_of_trackers], dtype = np.float32)
		...
		for index_det, detection in enumerate(detections):
			for index_track, tracker in enumerate(trackers):
				cosine_distance_matrix[index_det, index_track] = distance.cosine(detection[5:133], tracker['features'])
			min_dist = np.amin(cosine_distance_matrix[index_det, :])
			min_dist_index = np.argmin(cosine_distance_matrix[index_det, :])
			if (min_dist < self.reid_cosine_dist_threshold):
				row_ind.append(index_det)
				col_ind.append(min_dist_index)
		...
		# Now filtering out the matches after reid that have IOU less than the IOU threshold
		matches = []
		for m in matched_indices:
			row = m[0]
			col = m[1]
			iou_overlap = self.iou_individual(detections[row, 0:4], trackers[col]['coordinates'])
			if (iou_overlap < self.iou_threshold):
	...
	def step(self, frame, frame_num):
		...
		if self.do_reid:
			matches, unmatched_detections, unmatched_trackers = self.associate_detections_to_targets_with_reid(dets, self.trackers)
		else:
